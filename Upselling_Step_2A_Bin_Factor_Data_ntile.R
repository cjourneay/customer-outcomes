...
####################### BIN Factors for upselling  ############################################################# 

####### run the transformation on the NA-> None factor data set

ntile_results <- n_tile_transform("upselling", trainingdata_Factor_None_Transform, testdata_Factor_None_Transform)

write.table(ntile_results$ntile_training, sep="\t", na = "" ,  file = "upselling_Step_2A_train_Factor_None_Transform_ntile.csv", row.names = FALSE)
write.table(ntile_results$ntile_test, sep="\t", na = "",  file = "upselling_Step_2A_test_Factor_None_Transform_ntile.csv", row.names = FALSE)

####### run the transformation on the NA Mode imputed factor data set

ntile_results <- n_tile_transform("upselling", trainingdata_Factor_Mode_Transform, testdata_Factor_Mode_Transform)

write.table(ntile_results$ntile_training, sep="\t", na = "" ,  file = "upselling_Step_2A_train_Factor_Mode_Transform_ntile.csv", row.names = FALSE)
write.table(ntile_results$ntile_test, sep="\t", na = "",  file = "upselling_Step_2A_test_Factor_Mode_Transform_ntile.csv", row.names = FALSE)
...
save.image(file = "Upselling_Step_2A_factor_ntile_bin.RData")
